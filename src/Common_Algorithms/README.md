...
> 弗洛伊德算法是求最短路径的算法，用于计算一个结点到另一个结点的最短路径，最主要的特点是以起始点为中心向外层扩展，直到扩展到终点为止
>
> 弗洛伊德 VS 迪杰斯特拉:迪杰斯特拉算法通过选定的被访问顶点，求出从出发访问顶点到其他顶点的最短路径；弗洛伊德算法中每个顶点都是出发访问点，所以需要将每一个顶点看做是被访问点，求出每一个顶点到其他顶点的最短路径
>
> 弗洛伊德用于计算图中各个顶点之间的最短路径，迪杰斯特拉用于计算某一个顶点到其他顶点的最短路径
>
> 详见img.png
...
- 设置顶点vi到顶点vk的最短路径为Lik ，顶点vk到vj的最短路径为Ljk，顶点vj到vk的最短路径为Ljk，则vi到vj的最短路径为min{Lik+Ljk}
- vk的取值为图中的所有顶点，则可获得vi到vj的最短路径

## 马踏棋盘算法(HorseChessboard.java)
...
将马随机放在国际象棋的8X8的棋盘的某个方格中，马按照走日的方式前进，要求每个反方格只进入一次，走遍棋盘上的所有格子
...
- 将当前位置设置为已经访问，然后根据当前位置计算马能走的位置（走日的规则），并放入到一个集合(ArrayList)中，最多有8个位置，没走一步使用计数器计数
- 遍历ArrayList中存放的所有位置，看看哪个可以走得通，走通继续走，走不通就回溯
- 判断马是否完成了任务，使用计数器的值与应该走的步数比较，如果没有达到该走的数量，则没有完成任务，那么将整个棋盘置零